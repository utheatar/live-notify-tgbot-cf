...
import { BLStreamerBaseItem, BLStreamerItem, getBLStreamerStatusInfoList } from '../platforms/bilibili/aggregation';
...
export async function runTask_BL(env: Env): Promise<string> {
    ...
    const lastStatusRaw = await BLStore.getJson<BLStreamerItem[]>(KEY_LAST_STATUS) || [];
    // 转为 Map 方便 O(1) 查找: uid -> Status
    const lastStatusMap = new Map<number, BLStreamerItem>();
    ...
    // 准备新的缓存列表
    const newStatusCache: BLStreamerItem[] = [];
    // 收集数据库写入 Promise
    ...
    for (const curr of currentStatusList) {
        ...
        const currCache: BLStreamerItem = {
            uid: curr.uid,
            roomid: curr.room_id,
            name: curr.name,
        ...
        if (!last) {
            // 第一次抓取，写入初始记录
            needWrite = true;
        } else if (curr.live_status !== last.live_status) {
            // live_status 变化
            needWrite = true;
            updateKV_flag = true;
        } else if (curr.attention !== last.attention) {
            // attention 变化
            needWrite = true;
            updateKV_flag = true;
        } else if (curr.live_status === 1) {
            // 正在直播：每次都写入 DB 记录
            needWrite = true;
            // 仅在 title 或 guardNum 变化时更新 KV
            if (curr.title !== last.title || (curr.guard_count || 0) !== last.guardNum) {
                updateKV_flag = true;
            }
        }
