...
本项目提供了一个简单的本地数据分析工具，用于可视化 D1 数据库中积累的直播数据。目前打算每年进行一次数据分析，生成直播习惯和趋势的可视化报告。数据导出时间点为大年初一（农历），此时可以获得完整的前一年数据，且主播直播较少，对程序运行影响不大。
...
2. **生成前端数据**
   ...
   ```bash
   # 确保已安装 Python 3
   python src/analyze/parse_sql.py --sql [path/to/data.sql]
   ```
